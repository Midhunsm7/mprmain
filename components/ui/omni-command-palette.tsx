// components/ui/omni-command-palette.tsx
"use client";
    ...
    <Dialog.Root open={open} onOpenChange={setOpen}>
      <Dialog.Portal container={portalContainer ?? undefined}>
        ...
        <Dialog.Content
          ...
          className={cn(
            "fixed z-[101] inset-x-2 top-16 mx-auto w-[min(720px,100%-16px)] rounded-xl border bg-white dark:bg-gray-900 text-gray-900 dark:text-white shadow-lg backdrop-blur",
        ...
        >
            ...
            {renderHeader ? (
              ...
              <div className="flex items-center gap-2 px-3 py-2">
                ...
                <input
                  ...
                  className="flex-1 bg-transparent outline-none placeholder:text-gray-500 dark:placeholder:text-gray-400 text-sm"
                />
                <kbd className="rounded bg-gray-100 dark:bg-gray-800 px-1.5 py-0.5 text-[10px] text-gray-600 dark:text-gray-400">
                  ⌘K
                </kbd>
                <Dialog.Close asChild>
                  <button
                    aria-label="Close"
                    className="ml-2 rounded p-1 text-gray-500 dark:text-gray-400 hover:bg-gray-100 dark:hover:bg-gray-800"
                  >
          ...
          >
            {/* Loading bar */}
            {loadingIds.size > 0 && (
              <div className="flex items-center gap-2 px-3 py-2 text-gray-500 dark:text-gray-400 text-xs">
            ...
            {groups.map((g) => (
              <div key={g.id} className="py-1">
                {g.items.length > 0 && (
                  <div className="px-3 py-1 text-[10px] uppercase tracking-wider text-gray-500 dark:text-gray-400">
                    {g.label}
                  </div>
                )}
                    ...
                    return (
                      <button
                        ...
                        className={cn(
                          ...
                          active
                            ? "bg-cyan-100 dark:bg-cyan-900/30 text-gray-900 dark:text-white"
                      ...
                      >
                        {renderItem ? (
                          ...
                          <>
                            {/* Icon */}
                            <div className="shrink-0 text-gray-500 dark:text-gray-400 group-aria-selected:text-cyan-600 dark:group-aria-selected:text-cyan-400">
                            ...
                            <div className="min-w-0 flex-1">
                              ...
                              {item.subtitle && (
                                <div className="truncate text-xs text-gray-500 dark:text-gray-400 group-aria-selected:text-gray-700 dark:group-aria-selected:text-gray-300">
                                  {item.subtitle}
                                </div>
                              )}
                            </div>
                            ...
                            {item.shortcut && (
                              <span className="ml-2 hidden items-center gap-1 text-[10px] text-gray-500 dark:text-gray-400 sm:flex">
                                {item.shortcut.map((s, i) => (
                                  <kbd key={i} className="rounded bg-gray-100 dark:bg-gray-800 px-1 py-0.5">
                                    {s}
                                  </kbd>
                                ))}
                              </span>
                            )}
                            <ChevronRight className="ml-1 size-3.5 text-gray-500 dark:text-gray-400 opacity-0 group-hover:opacity-100" aria-hidden />
                          </>
                        )}
                      </button>
                    );
            ...
            {flatItems.length === 0 && (
              <div className="px-3 py-8 text-center text-sm text-gray-500 dark:text-gray-400">
                <div className="mx-auto mb-2 flex size-8 items-center justify-center rounded-full bg-gray-100 dark:bg-gray-800">
                  <History className="size-4" aria-hidden />
                </div>
                Try a different query, like "dashboard" or "bookings".
              </div>
            )}
          </div>
  ...
  for (let pos = 0; pos < label.length; pos++) {
    ...
    if (isHi) {
      ...
      out.push(
        <mark
          key={`m-${pos}-${run}`}
          className="rounded-[2px] bg-cyan-200 dark:bg-cyan-800 px-0.5 text-[inherit]"
        >
          {run}
        </mark>
      );
...
function ThemeIndicator() {
  ...
  return (
    <div className="inline-flex items-center gap-1 rounded px-2 py-1 text-gray-500 dark:text-gray-400">
